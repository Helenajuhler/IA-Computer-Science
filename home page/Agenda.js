  ...
  for (let i = 0; i < 6; i++) {
    ...
    for (let j = 0; j < 7; j++) {
      if (i === 0 && j < prevMonthDays) {
        const prevMonthDate = new Date(year, month - 2, daysInMonth - prevMonthDays + j + 1);
        ...
        calendarHTML += `<td class="current-month" data-date="${currentDate.toISOString().split('T')[0]}"><span class="day">${day}</span><div class="events"></div></td>`;
        day++;
      }
    }
    calendarHTML += '</tr>';
...
eventForm.addEventListener('submit', (e) => {
  ...
  const eventSelectedOption = eventInput.options[eventInput.selectedIndex].value;
  const eventData = { title: eventTitle, date: eventDate, time: eventTime, category: eventSelectedOption };
